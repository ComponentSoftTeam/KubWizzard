...
#Note: Not all Flags are used, only talk about the used ones
##Output:
#Chain of tought: 1) Uses the 'run' subcommand to start a pod\n2) provide the NAME 'nginx' for the pod\n3) Set the base image for the container with the '--image' flag to use the 'nginx' image
"""
]

example_prompt = '\n'.join([f'###Example {i+1}{p}' for i,p in enumerate(example_prompts)])
    
K8S_EXPERT = "You are a professinal developper that knows kubernetes and the kubectl cli."
def gen_cot(dataset):
    with tqdm(total=sum(len(x['examples']) for x in dataset), leave=True, desc="Generate CoT") as pbar:
        for data in dataset:
...
example_qi_prompt = '\n'.join([f'###Example {i+1}{p}' for i,p in enumerate(example_qi_prompts)])
...
def gen_qi(dataset):
    ...
    with tqdm(total=sum(len(x['examples']) for x in dataset), leave=True, desc="Generate questions") as pbar:
      ...
      for data in dataset:
          ...
          for example in examples:
            ...
            qis = [x for x in qis if not x.endswith(':')]
